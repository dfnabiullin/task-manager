...
class UserExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public ProblemDetail handleUserNotFoundException(UserNotFoundException ex, Locale locale) {
        ProblemDetail problemDetail = ProblemDetail.forStatus(HttpStatus.NOT_FOUND);
        problemDetail.setTitle("User Not Found");
        problemDetail.setDetail(messageSource.getMessage("error.user.notFound", new Object[]{ex.getUuid()}, locale));
    ...
    protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, @NonNull HttpHeaders headers, @NonNull HttpStatusCode status, @NonNull WebRequest request) {
        ...
        problemDetail.setDetail(ex.getAllErrors().stream().map(DefaultMessageSourceResolvable::getDefaultMessage)
